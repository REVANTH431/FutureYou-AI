// Backend copy of the scoring logic used in the frontend.
...
export function scoreSalary(input) {
  const countryFactor = (() => {
  ...
  const highLeverage = new Set(['AI/ML', 'System Design', 'Product Strategy', 'Leadership']);
  const skills = Array.isArray(input.skills) ? input.skills : [];
  const depthRaw =
    skills.length + skills.filter((s) => highLeverage.has(s)).length;
  ...
  const annualGrowthPct = clamp(
    0.03 + depth * 0.06 + consistencyNorm * 0.04,
    0.03,
    0.16,
  );
  ...
  for (let i = 0; i <= 10; i++) {
    ...
    current *= 1 + annualGrowthPct * (i > 0 ? 1 : 0.7);
  }
...
export function scoreHealth(input) {
  ...
  const sleep =
    input.studyHoursPerWeek == null ? 6.5 : clamp(input.studyHoursPerWeek / 3, 4, 9);
  let sleepDelta = 0;
  if (sleep < 6) score -= lerp(5, 18, (6 - sleep) / 2);
  else if (sleep > 8.5) score -= lerp(2, 8, (sleep - 8.5) / 1.5);
  else score += 8;
...
export function scoreStress(input) {
  let base = 40;
  switch (input.career) {
    case 'Sales / BizDev':
      base = 70;
  ...
  const overload = clamp((work - 40) / 25, 0, 1);
